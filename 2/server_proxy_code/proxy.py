...
def proxy(proxy_address: tuple[str, int], server_adress: tuple[str, int]) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_socket:
        ...
        # * Fill in start (1)
        proxy_socket.bind(proxy_address) # defines an ip address and port number for a socket instance
        ...
        while True:
            try:
                ...
                # * Fill in start (2)               
                client_socket, client_address = proxy_socket.accept()  # proxy accepts the client 
                # * Fill in end (2)
...
def client_handler(client_socket: socket.socket, client_address: tuple[str, int], server_address: tuple[str, int]) -> None:
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            # * Fill in start (3)
            data = client_socket.recv(api.BUFFER_SIZE) # data size : BUFFER_SIZE = 65536 = (2^16)
            # * Fill in end (3)

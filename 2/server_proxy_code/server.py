...
def server(host: str, port: int) -> None:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server_socket:
        ...
        # * Fill in start (1)
        server_socket.bind(("127.0.0.1",9999))  # set ip and port
        server_socket.listen(1)  #  the server is ready ro receives requests 
        # * Fill in end (1)
    ...
    with client_socket:  # closes the socket when the block is exited
        ...
        while True:
            ...
            try:
                ...
                # * Fill in start (4)
                client_socket.sendall(response) # sends the server's response (bytes)
                 # client socket close , connection close
                # * Fill in end (4)
